...
#include <GL/gl.h>
#include <GL/glu.h>
#include "SDL_image.h" // αつきの画像をロードするため
...
static void DrawSprite(GLuint texture, int imagew, int imageh, SpriteRect *src, SpriteRect *dest, int z, SpriteNode::ENUM_OP_TYPE op_type, const GLubyte color[4])
{
	...
	float fz = (float)z;
	float x = floorf(dest->x + 0.5f);
	float y = floorf(dest->y + 0.5f);
...
{
	if (!surface) {
		DebugPrintf("ImageLoader::AddSurface() surface for %s is NULL.\n", name);
		return false;
	}
...
// フォントを設定する
// 同時に、テキストモードに変更する
void SpriteNode::SetFont(TTF_Font *font)
{
	m_mode = MODE_TEXT;
	m_font = font;
}
...
bool DrawSystem::AddNode(SpriteNode *node)
{
	...
	if (!node->FetchImage()) {
		DebugPrint("SpriteNode::FetchImage() failed for:");
		DebugPrint(node->GetImageName());
	}
